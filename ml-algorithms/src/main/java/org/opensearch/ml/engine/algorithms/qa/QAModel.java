...
public class QAModel implements Predictable {
    ...
    private Predictor<QAInput, String>[] predictors;
    ...
    public MLOutput predict(MLInput mlInput) {
        ...
        return predictQA(modelId, mlInput.getInputDataset());
    }
    ...
    protected void loadQAModel(File modelZipFile, String modelId, String modelName, FunctionName functionName, String version,
                               MLModelConfig modelConfig,
                               String engine) {

        try {
            if (FunctionName.QUESTION_ANSWERING != functionName) {
            ...
            List<Predictor<QAInput, String>> predictorList = new ArrayList<>();
            List<ZooModel<QAInput, String>> modelList = new ArrayList<>();
            AccessController.doPrivileged((PrivilegedExceptionAction<Void>) () -> {
                ...
                try {
                    ...
                    for (int i = 0; i < devices.length; i++) {
                        ...
                        Criteria.Builder<QAInput, String> criteriaBuilder = Criteria.builder()
                                .setTypes(QAInput.class, String.class)
                                .optApplication(Application.UNDEFINED)
                                .optEngine(engine)
                                .optDevice(devices[i])
                                .optModelPath(modelPath)
                                .optTranslator(new PtQATranslator());
...
//                            }
//                        }
                        Criteria<QAInput, String> criteria = criteriaBuilder.build();
    ...
    protected ModelTensorOutput predictQA(String modelId, MLInputDataset inputDataSet) {
        try {
            return AccessController.doPrivileged((PrivilegedExceptionAction<ModelTensorOutput>) () -> {
                ...
                QAInput input = new QAInput(question, context);
                String output = predictors[currentDevice].predict(input);
    ...
    protected ModelTensors parseModelTensorOutput(String output, ModelResultFilter resultFilter) {
        if (output == null) {
        ...
        String name = "answer";
        modelTensors.add(new ModelTensor(name, output));
        ModelTensors tensorOutput = new ModelTensors(modelTensors);
