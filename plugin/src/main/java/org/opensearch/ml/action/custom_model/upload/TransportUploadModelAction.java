        ...
        mlTaskManager.createMLTask(mlTask, ActionListener.wrap(response -> {
            ...
            mlTaskDispatcher.dispatch(ActionListener.wrap(nodeId -> {
                if (clusterService.localNode().getId().equals(nodeId)) {
                    ...
                    log.info("Upload model {} locally on node {}", mlUploadInput.getName(), nodeId);
                    mlModelUploader.uploadModel(mlUploadInput, mlTask);
                } else {
                    // Execute ML task remotely
                    log.info("Upload model {} remotely on node {}", mlUploadInput.getName(), nodeId);
                    MLForwardRequest forwardRequest = new MLForwardRequest(
                        new MLForwardInput(
                            ...
                            null,
                            mlUploadInput.getModelFormat(), mlUploadInput.getModelConfig(), null, null
                        )
                    );
