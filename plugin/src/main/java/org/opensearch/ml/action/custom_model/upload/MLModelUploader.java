...
//                                .modelContentSizeInBytes(modelSizeInBytes)
//                                .modelContentMD5(md5)
//                                .totalChunks(chunkFiles.size())
                                .modelConfig(mlUploadInput.getModelConfig())//TODO: check if getModelConfig is null
                                .createdTime(Instant.now())
                                .build();
                        System.out.println("aaaaaaaaaaaa mlUploadInput.getModelConfig() = " + mlUploadInput.getModelConfig().toString());
                            ...
                            customModelManager.downloadAndSplit(modelId, modelName, version, mlUploadInput.getUrl(),
                                    ActionListener.wrap(result -> {
                                        ...
                                        for (String name : chunkFiles) {
                                            semaphore.tryAcquire(10, TimeUnit.SECONDS);//TODO: change to tryAcquire?
...
//                                indexRequest.id(MLModel.customModelId(modelName, version, chunkNum));
                                            ...
                                            client.index(indexRequest, ActionListener.wrap(r -> {
                                                ...
                                                if (uploaded.get() == chunkFiles.size()) {
                                                    ...
                                                    mlModelManager.updateModel(modelId,
                                                            ImmutableMap.of(MLModel.MODEL_STATE_FIELD, MLModelState.UPLOADED,
                                                                    ...
                                                                    MLModel.MODEL_CONTENT_HASH_FIELD, md5,
                                                ...
                                                // mlTaskManager.updateMLTask();// TODO: track uploading progress
                                                semaphore.release();
                                            }, e -> {
...
//                customModelManager.downloadAndSplit(modelName, version, mlUploadInput.getUrl(), ActionListener.wrap(result -> {
//                    Long modelSizeInBytes = (Long) result.get(MODEL_SIZE_IN_BYTES);
//                    List<String> chunkFiles = (List<String>) result.get(CHUNK_FILES);
//                    String md5 = (String) result.get(MODEL_FILE_MD5);
//
...
//                                            .version(version)
//                                            .modelFormat(mlUploadInput.getModelFormat())
//                                            .modelTaskType(MLModelTaskType.TEXT_EMBEDDING) //TODO: TODO: get this from mlUploadInput
//                                            .chunkNumber(chunkNum)
//                                            .totalChunks(chunkFiles.size())
//                                            .content(Base64.getEncoder().encodeToString(bytes))
//                                            .createdTime(Instant.now())
...
//
//                                    client.index(indexRequest, ActionListener.wrap(r -> {
//                                        log.debug("Index model chunk successfully {}, {}", modelName, name);
//                                        uploaded.getAndIncrement();
//                                        if (uploaded.get() == chunkFiles.size()) {
//                                            mlTaskManager
//                                                    .updateMLTask(
...
//                                            file.getParentFile().delete();
//                                            mlTaskManager.remove(taskId);
//                                            mlModelManager.updateModel(modelId, ImmutableMap.of(MLModel.MODEL_STATE_FIELD, MLModelState.UPLOADED, MLModel.LAST_UPLOADED_TIME_FIELD, Instant.now().toEpochMilli()));
//                                        } else {
//                                            file.delete();
//                                        }
//                                        // mlTaskManager.updateMLTask();// TODO: track uploading progress
...
//                    } catch (Exception e) {
//                        //TODO: remove duplicate code
//                        log.error("Failed to stash thread context", e);
//                        mlTaskManager
//                                .updateMLTask(
//                                        taskId,
//                                        ImmutableMap
...
//    public void updateModel(String modelId, ImmutableMap<String, Object> updatedFields, int timeoutInMillis) {
...
//            }
//        }, e -> { log.error("Failed to update ML model: " + modelId, e); }), timeoutInMillis);
//    }
//
//    public void updateModel(String modelId, ImmutableMap<String, Object> updatedFields,
//                            ActionListener<UpdateResponse> listener, int timeoutInMillis) {
//        try {
