...
public class TransportForwardAction extends HandledTransportAction<ActionRequest, MLForwardResponse> {
    ...
    protected void doExecute(Task task, ActionRequest request, ActionListener<MLForwardResponse> listener) {
        ...
        log.info("ylwudebug2 --- receive forward request: " + forwardInput.getRequestType());
        try {
            switch (requestType) {
    ...
    private void syncModelWorkerNodes(String modelId) {
        ...
        log
            .info(
                "ylwudebug22 {}--- sycn model {} workernodes: {}",
                clusterService.localNode().getId(),
                modelId,
                Arrays.toString(workerNodes)
            );
        if (allNodes.length > 1 && workerNodes.length > 0) {
            log.info("ylwudebug2 --- sync model routing to other nodes. model loaded on nodes: {}", Arrays.toString(workerNodes));
            ...
            client
                .execute(
                    ...
                    ActionListener
                        .wrap(
                            r -> { log.info("ylwudebug2 --- Sync up successfully"); },
                            e -> { log.error("ylwudebug2 --- Failed to sync up", e); }
                        )
                );
        }
    }
