    ...
    public void dispatchTask(Request request, TransportService transportService, ActionListener<Response> listener) {
        mlTaskDispatcher.dispatchTask(ActionListener.wrap(node -> {
            if (clusterService.localNode().getId().equals(node.getId())) {
                ...
                transportService.sendRequest(node, getTransportActionName(), request, getResonseHandler(listener));
            }
        }, e -> listener.onFailure(e)));
    }

    public abstract String getTransportActionName();

    public abstract TransportResponseHandler<Response> getResonseHandler(ActionListener<Response> listener);

    public abstract void executeTask(Request request, ActionListener<Response> listener);
}
