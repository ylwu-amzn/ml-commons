...
public final class MemoryContainerPipelineHelper {
        ...
        try {
            if (config.getRemoteStore().getEmbeddingModelType() != null) {
                ...
                createRemoteTextEmbeddingPipeline(connectorId, pipelineName, config, client, ActionListener.wrap(success -> {
                    ...
                    org.opensearch.ml.helper.RemoteStorageHelper.createRemoteLongTermMemoryIndexWithPipeline(
                        connectorId,
                        indexName,
                        pipelineName,
                        config,
                ...
                org.opensearch.ml.helper.RemoteStorageHelper.createRemoteLongTermMemoryIndex(
                    connectorId,
                    indexName,
                    config,
                    indicesHandler,
    ...
    public static void createRemoteTextEmbeddingPipeline(
        ...
        try {
            ...
            // Use remote embedding model ID if specified, otherwise use local embedding model ID
            String embeddingModelId = remoteStore.getEmbeddingModelId();
//            if (config.getRemoteStore() != null && config.getRemoteStore().getEmbeddingModelId() != null) {
//                embeddingModelId = config.getRemoteStore().getEmbeddingModelId();
//                log.info("Using remote embedding model ID for pipeline: {}", embeddingModelId);
//            } else {
//                log.info("Using local embedding model ID for pipeline: {}", embeddingModelId);
//            }
            ...
            org.opensearch.ml.helper.RemoteStorageHelper.createRemotePipeline(
                connectorId,
                pipelineName,
                pipelineBody,
                client,
