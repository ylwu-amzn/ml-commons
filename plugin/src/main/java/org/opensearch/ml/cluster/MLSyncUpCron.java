...
public class MLSyncUpCron implements Runnable {
    ...
    public MLSyncUpCron(Client client, ClusterService clusterService, DiscoveryNodeHelper nodeHelper, MLIndicesHandler mlIndicesHandler, MLEngine mlEngine) {
        this.client = client;
    ...
    void initMLConfig() {
        ...
        if (!clusterService.state().metadata().indices().containsKey(ML_CONFIG_INDEX)) {
            mlIndicesHandler.initMLConfigIndex(ActionListener.wrap(r -> {
                ...
                client.get(getRequest, ActionListener.wrap(getResponse -> {
                    if (!getResponse.isExists()) {
                        ...
                        String masterKey = mlEngine.generateMasterKey();
                        indexRequest.source(ImmutableMap.of(MASTER_KEY, masterKey));
                ...
                log.error("Failed to init ML config index", e);
            }));
        }
    }
