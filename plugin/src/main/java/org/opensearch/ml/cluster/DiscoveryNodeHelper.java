...
public class DiscoveryNodeHelper {
    ...
    private volatile Set<String> remoteModelRunOn;
    private volatile Set<String> localModelRunOn;
    ...
    public DiscoveryNodeHelper(ClusterService clusterService, Settings settings) {
        ...
        remoteModelRunOn = new HashSet<>();
        remoteModelRunOn.addAll(ML_COMMONS_TASK_DISPATCHER_REMOTE_MODEL.get(settings));
        clusterService.getClusterSettings().addSettingsUpdateConsumer(ML_COMMONS_TASK_DISPATCHER_REMOTE_MODEL, it -> {
            remoteModelRunOn = new HashSet<>(it);
        });
        localModelRunOn = new HashSet<>();
        localModelRunOn.addAll(ML_COMMONS_TASK_DISPATCHER_LOCAL_MODEL.get(settings));
        clusterService.getClusterSettings().addSettingsUpdateConsumer(ML_COMMONS_TASK_DISPATCHER_LOCAL_MODEL, it -> {
            localModelRunOn = new HashSet<>(it);
        });
    }
    ...
    public DiscoveryNode[] getEligibleNodes(FunctionName functionName) {
        ...
        for (DiscoveryNode node : state.nodes()) {
            ...
            if (functionName == FunctionName.REMOTE) {// remote model
                getEligibleNodes(remoteModelRunOn, eligibleNodes, node);
            } else { // local model
                if (onlyRunOnMLNode) {
                    ...
                    getEligibleNodes(localModelRunOn, eligibleNodes, node);
                }
            }
        }
