...
public class MLUploadModelMetaInput implements ToXContentObject, Writeable{
    ...
    public static final String MODEL_TASK_TYPE_FIELD = "model_task_type"; // only text_embedding is supported for now
    ...
    // Algorithm name
    private FunctionName algorithm = FunctionName.TEXT_EMBEDDING;
    private String name;
    ...
    public MLUploadModelMetaInput(String name, FunctionName algorithm, Integer version,  String description, MLModelFormat modelFormat, MLModelTaskType modelTaskType, MLModelState modelState, Long modelContentSizeInBytes, String modelContentHash, MLModelConfig modelConfig, Integer totalChunks) {
        this.name = name;
        this.algorithm = FunctionName.TEXT_EMBEDDING;
    ...
    public MLUploadModelMetaInput(StreamInput in) throws IOException{
        ...
        if (in.readBoolean()) {
            modelTaskType = in.readEnum(MLModelTaskType.class);
        }
        if (in.readBoolean()) {
